...
.noPit:
    ...
    bcs    .doQuickSand     ; 2ł                 yes, animate quicksand
    cmp    #JUNGLE_GROUND+1 ; 2                 stop quicksand animation when..
    bcs    .stopQuickSand   ; 2ł                ..Harry is falling into the pit
.doQuickSand:
    lda    noGameScroll     ; 3                 game running?
    bne    .stopQuickSand   ; 2ł                 no, skip
    lda    frameCnt         ; 3
    lsr                     ; 2
    lsr                     ; 2
    pha                     ; 3
...
; calculate x-positioning values:
  ...
  IF SCREENSAVER
    ...
    bmi    .skipHarryPos    ; 2ł                 no, don't draw Harry
  ELSE
    ldy    noGameScroll     ;                   TODO: bugfix, wall isn't drawn
    bne    .skipHarryPos    ; 2ł                 no, don't draw Harry
  ENDIF
...
; set digitPtrs for timer:
    ...
    sta    digitPtr         ; 3
    ldy    digitPtr+2       ; 3
    bne    .noSpace         ; 2ł                replaced leading zero in timer with space
    sta    digitPtr+2       ; 3
.noSpace:
    lda    #<DoublePoint    ; 2
    sta    digitPtr+6       ; 3
...
;---------------------------------------
    ...
    sta    GRP0             ; 3
    sta    GRP1             ; 3
    bcs    .noChangePF      ; 2ł                two line resolution for leaves
...
;---------------------------------------
    ...
    sta    GRP1             ; 3
    dey                     ; 2
    bmi    .exitHoles       ; 2ł                exit loop here
    lda    PF2Lst,x         ; 4
    sta    PF2              ; 3
    inx                     ; 2
...
;---------------------------------------
    ...
    sta    ENABL            ; 3
    dex                     ; 2
    bmi    .exitLadderTop   ; 2ł                exit loop
    lda    temp3            ; 3
    sta    HMCLR            ; 3
...
.loopCopyright:
    ...
    dex                     ; 2
    dex                     ; 2
    bpl    .loopCopyright   ; 2ł

    lda    colorLst+4       ; 3
    sta    COLUPF           ; 3
    jsr    ShowDigits       ; 6
    lda    noGameScroll     ; 3                 game running?
    beq    .endCopyright    ; 2ł                 yes, no more scrolling
...
; check for killed Harry:
    ...
    bne    .slipDecrease    ; 2ł                 no, skip decrease
; Harry is loosing a live:
    lda    livesPat         ; 3                 any more lives?
    beq    .slipDecrease    ; 2ł                 no, skip decrease
  IF TRAINER
    FILL_NOP 4
  ELSE
...
.loopBounds:
    ...
    beq    .exitBounds      ; 2ł                no more bounds!
    clc                     ; 2
    adc    xPosQuickSand    ; 3
    cmp    xPosHarry        ; 3                 Harry left of hole/pit?
    bcs    .inBounds        ; 2ł                 yes, bound ok
    lda    HoleBoundsTab+1,x; 4
    sec                     ; 2
    sbc    xPosQuickSand    ; 3
    cmp    xPosHarry        ; 3                 Harry right of hole/pit?
    bcs    .outOfBounds     ; 2ł                 no, Harry is falling into
.inBounds:
    inx                     ; 2
    inx                     ; 2
    dey                     ; 2
...
.noReset:
    lda    noGameScroll     ; 3                 game running?
    beq    .processHarry    ; 2ł                 yes, process Harry
...
.processHarry:
    ...
    rol    random2          ; 5
    lda    climbPos         ; 3                 Harry at ladder?
    bne    .endDoJump       ; 2ł+1               no, skip continue jump
    ldx    jumpIndex        ; 3                 currently jumping?
    beq    .endDoJump       ; 2ł+1               no, skip continue jump
...
.indexOk:
    ...
    beq    .stopJump        ; 2ł+1               yes, stop any jump
    ldy    ladderFlag       ; 3                 ladder in scene?
    beq    .skipFalling     ; 2ł+1               no, skip falling
    cpx    #JUNGLE_GROUND+2 ; 2
    bne    .skipFalling     ; 2ł+1
...
.skipFalling:
    ...
    beq    .stopJump        ; 2ł                 yes, stop any jump
    cpx    #54              ; 2                 has Harry reached the falling limit?
    bne    .endDoJump       ; 2ł                 no, skip
    tya                     ; 2                 ladder in scene?
    bne    .endDoJump       ; 2ł                 no, skip kill
...
.contCollision:
    ...
    bcs    .checkWallHit    ; 2ł                 yes, check wall
    lda    atLiana          ; 3                  no, Harry at liana?
    bne    .endCollision    ; 2ł                 yes, skip
    lda    sceneType        ; 3
    cmp    #CROCO_SCENE     ; 2                 croco in scene?
    beq    .endCollision    ; 2ł                 yes, skip
    cmp    #TREASURE_SCENE  ; 2                 treasue in scene?
    bne    .noTreasure1     ; 2ł                 no, skip
...
.noTreasure1:
    ...
    bcc    .hitLogs         ; 2ł                 no, hit by rolling logs
.noWallHit:
    jmp    KilledHarry      ; 3                 Harry is killed

.hitLogs:
    lda    climbPos         ; 3                 Harry at ladder?
    beq    .notAtLadder     ; 2ł                 no, skip push
...
; calculate bottom of liana:
    ...
    lsr                     ; 2
    cmp    #6-1             ; 2
    bcs    .limitBottom     ; 2ł                limit bottom of liana to 6
    lda    #6               ; 2
.limitBottom:
    adc    #4               ; 2
    sta    lianaBottom      ; 3
...
; check for jumping of liana:
    ...
    beq    .skipJumpOff     ; 2ł                 no, skip jump of liana
    lda    joystick         ; 3
    and    #~[$f0|MOVE_DOWN]; 2                 joystick down?
    bne    .skipJumpOff     ; 2ł                 no, skip
...
; check for starting climbing ladder:
    lda    climbPos         ; 3                 Harry at ladder?
    bne    .endStartClimb   ; 2ł                 yes, skip
    lda    ladderFlag       ; 3                 ladder in scene?
    beq    .endStartClimb   ; 2ł                 no, skip
    lda    xPosHarry        ; 3
    sec                     ; 2
    sbc    #68              ; 2
    cmp    #15              ; 2                 Harry at x-position of ladder (+/-7)?
    bcs    .endStartClimb   ; 2ł                 no, skip
    lda    yPosHarry        ; 3                  yes,
    cmp    #84              ; 2                 Harry near bottom of underground
    bcc    .skipClimbUp     ; 2ł                 no, skip
    lda    joystick         ; 3                  yes,
    lsr                     ; 2                 joystick up?
    bcs    .skipClimbUp     ; 2ł                 no, skip
...
; move Harry when swinging at liana:
    lda    atLiana          ; 3                 Harry at liana?
    beq    .skipSwingHarry  ; 2ł                 no, skip moving Harry
...
; check Harry climbing ladder:
    lda    climbPos         ; 3                 Harry at ladder?
    beq    .endClimbLadder  ; 2ł                 no, skip
    lda    #0               ; 2
    sta    jumpIndex        ; 3
    lda    frameCnt         ; 3
    and    #$07             ; 2                 climb every 8th frame
    bne    .skipAnimClimb   ; 2ł
    lda    joystick         ; 3
    lsr                     ; 2                 joystick up?
    bcs    .notClimbUp      ; 2ł                 no, skip
    dec    climbPos         ; 5                  yes, climb up
.notClimbUp:
    lsr                     ; 2                 joystick down?
    bcs    .notClimbDown    ; 2ł                 no, skip
    inc    climbPos         ; 5                  yes, climb down
.notClimbDown:
    lda    climbPos         ; 3
    cmp    #LADDER_TOP      ; 2                 top reached?
    bcs    .skipLadderTop   ; 2ł                 no, skip
    lda    #NO_MOVE         ; 2
    sta    oldJoystick      ; 3
    lda    #LADDER_TOP      ; 2
.skipLadderTop:
    cmp    #LADDER_BOTTOM   ; 2                 bottom reached?
    bcc    .skipLadderBottom; 2ł                 no, skip
...
; animate running Harry:
    lda    atLiana          ; 3                 Harry at liana?
    bne    .endHarryId      ; 2ł+1               yes, skip running
    lda    climbPos         ; 3
    cmp    #LADDER_TOP+1    ; 2                 Harry at ladder bottom?
    bcs    .endHarryId      ; 2ł+1               no, skip running
    ...
    bne    .isJumping       ; 2ł                 yes, use old joystick input
    lda    joystick         ; 3                  no, use new joystick input
.isJumping:
    lsr                     ; 2
    lsr                     ; 2
    lsr                     ; 2                 joystick left?
    bcs    .skipLeft        ; 2ł+1               no, skip
    dec    xPosHarry        ; 5
    ldy    #REFLECT         ; 2
    sty    reflectHarry     ; 3
    cpx    #0               ; 2                 4th frame?
    bne    .skipAnimLeft    ; 2ł                 no, skip animation
    dec    patIdHarry       ; 5
.skipAnimLeft:
    jmp    .endAnimHarry    ; 3
...
.skipLeft:
    ...
    bcs    .endAnimHarry    ; 2ł                 no, skip
    inc    xPosHarry        ; 5
    ldy    #NOREFLECT       ; 2
    sty    reflectHarry     ; 3
    cpx    #0               ; 2                 4th frame?
    bne    .endAnimHarry    ; 2ł                 no, skip animation
    dec    patIdHarry       ; 5
.endAnimHarry:
...
; goto next scene, if Harry has reached border of current scene:
    ...
    lda    yPosHarry        ; 3
    cmp    #64              ; 2                 Harry at underground?
    bcc    .oneScene        ; 2ł                 yes, move one scene
...
; move the scorpion towards harry:
    lda    ladderFlag       ; 3                 ladder in scene?
    bne    .noMoveScorpion  ; 2ł                 yes, skip scorpion
    ldx    #NOREFLECT       ; 2
    lda    xPosHarry        ; 3
    sec                     ; 2
    sbc    xPosScorpion     ; 3
    beq    .noMoveScorpion  ; 2ł
    bcs    .rightOfScorpion ; 2ł
    ldx    #REFLECT         ; 2
.rightOfScorpion:
    lda    frameCnt         ; 3
    and    #$07             ; 2                 move scorpion every 8th frame
    bne    .endMoveScorpion ; 2ł
    inc    xPosScorpion     ; 5
    bcs    .endMoveScorpion ; 2ł
    dec    xPosScorpion     ; 5
    dec    xPosScorpion     ; 5
.endMoveScorpion:
...
.noMoveScorpion:
    ...
    bne    .notAtTop        ; 2ł                 no, skip horizontal move
    lda    joystick         ; 3
    and    #JOY_HORZ        ; 2
    cmp    #JOY_HORZ        ; 2                 joystick left or right?
    beq    .notAtTop        ; 2ł                 no, skip
    lda    joystick         ; 3
    sta    oldJoystick      ; 3
    lda    #1               ; 2
    sta    jumpIndex        ; 3
...
.notOfTop:
    ...
    bcs    .contPatId       ; 2ł
    lda    climbPos         ; 3                 Harry at ladder?
    bne    .contPatId       ; 2ł                 yes, skip
    ldx    #ID_STANDING     ; 2                  no, draw standing Harry
.contPatId:
    lda    atLiana          ; 3                 Harry at liana?
    beq    .skipStanding2   ; 2ł                 no, skip
    ldx    #ID_SWINGING     ; 2                  yes, draw swinging Harry
.skipStanding2:
    lda    climbPos         ; 3                 Harry at ladder?
    beq    .noAnimClimb     ; 2ł                 no, skip
...
.noCroco:
    lda    noGameScroll     ; 3                 game running?
    bne    .skipLogs        ; 2ł                 no, skip
    lda    CrocoTab,y       ; 4                 crocos in scene?
    bne    .skipLogs        ; 2ł                 no, skip
    cpy    #TREASURE_SCENE  ; 2                 treasure in scene?
    beq    .skipLogs        ; 2ł                 yes, skip
...
; animate, bounce and move rolling logs:
    ...
    bcs    .skipLogs        ; 2ł                 no, skip move
    ldx    xPosObject       ; 3
    bne    .skipResetLogs   ; 2ł
    ldx    #SCREENWIDTH     ; 2
.skipResetLogs:
    dex                     ; 2
    stx    xPosObject       ; 3
